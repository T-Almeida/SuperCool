...
THREE.MD2Character = function () {
	...
	function createPart( geometry, skinMap ) {
		...
		var materialWireframe = new THREE.MeshLambertMaterial( { color: 0xffaa00, emissive: 0xffff00, wireframe: true, morphTargets: true, morphNormals: true } );
	...
	/**
	 * Função criada para permitir copiar MD2Character já criado sem ter de carregar sempre os modelos.
	 */
	this.copy = function(character) {
		this.onLoadComplete = character.onLoadComplete;
		this.loadCounter = character.loadCounter;
		this.scale = character.scale;
		this.animationFPS = character.animationFPS;
		this.root = character.root.clone();

		if (character.meshBody) this.meshBody = this.root.children[0];
		if (character.meshWeapon) this.meshWeapon = character.meshWeapon.clone();
	
		this.skinsBody = character.skinsBody;
		this.skinsWeapon = character.skinsWeapon;
		this.weapons = character.weapons;
	
		if (character.activeAnimation) this.activeAnimation = character.activeAnimation;
		//this.root.children[0] = this.meshBody;
		this.mixer =  new THREE.AnimationMixer( this.meshBody );
